...
(function () {
  ...
  function extract() {
    ...
    const catalog = text('.product-code, [data-testid="catalog-no"]');
    const sizeEl = document.querySelector('.product-package-size, .pack-size');
    const size = sizeEl ? sizeEl.textContent.trim() : '';
    const priceEl = document.querySelector('.price-value, .product-price');
